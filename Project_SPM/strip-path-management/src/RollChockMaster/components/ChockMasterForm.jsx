...
import { fetchExistingChock, saveChock } from "../services/chockLookupService";
...
export default function ChockMasterForm({ lookups }) {
  ...
  const handleQuery = async () => {
    ...
    if (existing) {
      ...
      setStatusDesc(lookups?.statusDesc?.[existing.CHM_CD_CHK_PROG] ?? "");
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const result = await saveChock(form);
      if (result.success) {
        alertify.success(isExistingRecord ? "Record Updated" : "Record Inserted");
      }
    } catch (err) {
      alertify.error(`Record Failed: ${err.message}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const renderWidthRow = (row, prefix) => {
    ...
    return (
      <div className="crf-width-row" key={inField}>
        ...
        <div className="crf-width-cell">
          {row.hasTolerance && (
            <>
              <input className="crf-input crf-input--num crf-input--disabled crf-input--limit" disabled />
              <span className="crf-unit">(mm)</span>
            </>
          )}
        </div>
    ...
    <div className="crf-screen">
      ...
      <fieldset className="crf-panel crf-panel--inner">
        ...
        <div className="crf-info-grid">
          ...
          <input className="crf-input crf-input--wide crf-chockmaker" value={form.CHM_CHK_MAKER} onChange={set("CHM_CHK_MAKER")} />
        </div>
      </fieldset>
      ...
      <div className="crf-row-panels">
        ...
        <fieldset className="crf-panel crf-panel--inner crf-panel--half">
          ...
          {WIDTH_ROWS.map((r) => renderWidthRow(r, WITHOUT_LINER_PREFIX))}
        </fieldset>
        ...
        <fieldset className="crf-panel crf-panel--inner crf-panel--half">
          ...
          {WIDTH_ROWS.map((r) => renderWidthRow(r, WITH_LINER_PREFIX))}
        </fieldset>
      </div>
